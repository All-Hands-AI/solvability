...
def featurizer(mock_completion_response, features) -> Featurizer:
    ...
    pytest.MonkeyPatch().setattr(
        "solvability.models.featurizer.completion", lambda *args, **kwargs: mock_completion_response
    )

    featurizer = Featurizer(
        system_prompt="Test system prompt", message_prefix="Test message prefix: ", features=features
    )
...
def solvability_classifier(featurizer, mock_classifier):
    ...
    return SolvabilityClassifier(
        identifier="test-classifier", featurizer=featurizer, classifier=mock_classifier, random_state=42
    )
