...
def test_transform_produces_feature_columns(solvability_classifier):
    """Test transform method produces expected feature columns."""
    issues = pd.Series(["Test issue"])
    features = solvability_classifier.transform(issues)
...
def test_transform_sets_classifier_attrs(solvability_classifier):
...
def test_fit_sets_classifier_attrs(solvability_classifier):
    """Test fit method sets classifier attribute `feature_importances_`."""
    issues = pd.Series(["Test issue"])
    labels = pd.Series([1])

    # Fit the classifier
    solvability_classifier.fit(issues, labels)
...
def test_predict_proba_sets_classifier_attrs(solvability_classifier):
    ...
    # effects.
    _ = solvability_classifier.predict_proba(issues)
...
def test_predict_sets_classifier_attrs(solvability_classifier):
    ...
    _ = solvability_classifier.predict(issues)
...
def test_importance_strategies(strategy, solvability_classifier):
...
def test_is_fitted_property(solvability_classifier):
...
def test_solvability_report_well_formed(solvability_classifier):
    ...
    report = solvability_classifier.solvability_report(issues.iloc[0])
